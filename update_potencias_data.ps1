...
# We define a hashtable with all the data
$potencias = @{
...
# 1. Remove all existing potencia_regional blocks
$html = [System.Text.RegularExpressions.Regex]::Replace($html, 'potencia_regional:\s*\{[^}]+\},?\s*', '')

# 2. Iterate and add the new blocks correctly
foreach ($country in $potencias.Keys) {
    $data = $potencias[$country]
